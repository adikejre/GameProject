...
Dinosaur.prototype.draw = function(context) {
    ...
    // var oldFill = context.fillStyle;
     // var dino_img = document.getElementById("t-rex")
     // var pat = context.createPattern(dino_img, 'no-repeat');
...
function Game () {
    ...
    document.addEventListener("keydown", function(e) {
        if (e.key === " " || e.key == 'ArrowUp' || e.key=='Enter') this.spacePressed = true;
    });
    document.addEventListener("keyup", function(e) {
        if (e.key === " " || e.key == 'ArrowUp' || e.key=='Enter') this.spacePressed = false;
    });
    this.gravity = 1.5;
    ...
    for(i = 0; i < this.cacti.length; i++){
        if(
            ...
            && leftWall(this.dino) <= rightWall(this.cacti[i]) && bottomWall(this.dino) >= topWall(this.cacti[i]))
            {
                ...
                if(this.paused)
                {
                  message.innerHTML = 'Press Backspace To Restart';
                }
                ...
                $(document).ready(function() {
                  ...
                  $.ajax({
                    ...
                    url:'/savescoredino/',
                    data:{
                      'final_score':score_val.innerHTML,
